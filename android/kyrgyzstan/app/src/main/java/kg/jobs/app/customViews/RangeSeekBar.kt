...
open class RangeSeekBar : View {
    ...
    private val trackPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        ...
        pathEffect = CornerPathEffect(6.toPx.toFloat())
    }
    ...
    private val trackSelectedPath = Path()
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawPath(trackPath.apply {
            reset()
            moveTo(paddingLeft + 0f, verticalCenter - trackThickness / 2)
            lineTo(paddingLeft + width.toFloat(), verticalCenter - trackThickness / 2)
            lineTo(paddingLeft + width.toFloat(), verticalCenter + trackThickness / 2)
            lineTo(paddingLeft + 0f, verticalCenter + trackThickness / 2)
            close()
        }, trackPaint.apply { color = trackColor })

        // Draw selected range of the track
        canvas.drawPath(trackSelectedPath.apply {
            reset()
            moveTo(minimumX, verticalCenter - trackSelectedThickness / 2)
            lineTo(maximumX, verticalCenter - trackSelectedThickness / 2)
            lineTo(maximumX, verticalCenter + trackSelectedThickness / 2)
            lineTo(minimumX, verticalCenter + trackSelectedThickness / 2)
            close()
        }, trackSelectedPaint.apply {
            shader = LinearGradient(0f, 0f, width.toFloat(), height.toFloat(),
                    Color.parseColor("#5F3C9F"),
                    Color.parseColor("#FFAF7B"), Shader.TileMode.MIRROR)
        })
        ...
        if (!disable) maxThumbDrawable.drawAtPosition(canvas, maximumX.toInt() - maxThumbDrawable.intrinsicWidth)
    }

    private var disable = false
    fun disableRange(disable: Boolean) {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                } else if (!disable && isInsideRadius(event, rightThumbX, height / 2, touchRadius)) {
