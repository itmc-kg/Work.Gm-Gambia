    ...
    private var vacancy : Vacancy? {
        didSet{
            if let vacancy = vacancy{
                ...
                applyLabel.text = "applyLabel".localized()
                skipLabel.text = "dissmisLabel".localized()
                ...
                if language == "en" {
                    ...
                    employmentLabel.text = Constants.SHEDULE_NAMES_EN[scheduleIndex]
                } else if language == "ky" {
                    scheduleLabel.text = Constants.EMPLPOYMENT_TYPES_KG[employmentTypeIndex]
                    employmentLabel.text = Constants.SHEDULE_NAMES_KG[scheduleIndex]
                } else {
                    scheduleLabel.text = Constants.EMPLPOYMENT_TYPES[employmentTypeIndex]
                    employmentLabel.text = Constants.SHEDULE_NAMES[scheduleIndex]
                }
                ...
                if let application = vacancy.application{
                    ...
                    if application.status == ApplicationStatus.Accepted.rawValue {
                        self.applyLabel.text = "sendMessage".localized()
                    } else {
                        self.applyLabel.text = "applyLabel".localized()
                    }
                }else{
