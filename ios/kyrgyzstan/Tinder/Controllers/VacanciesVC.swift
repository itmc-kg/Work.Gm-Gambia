...
//  Tinder
//
//  Created by Azamat Kushmanov on 9/20/18.
...
class VacanciesVC: UIViewController {
    ...
    private var undoVacancy : Vacancy?{
        didSet{
            ...
            if let vacancy = undoVacancy, let employer = vacancy.employer {
                undoButton.sd_setImage(with: employer.getImageUrl(),for: .normal, placeholderImage: Constants.placeholder, options: .continueInBackground, completed: nil)
    ...
    func isSupportFilter(vacancy: Vacancy) -> Bool{
        ...
        //SANTINEET
        if let user = Auth.auth().currentUser, user.uid == vacancy.creatorId {
            return false
        }
    ...
    func getVacancies(){
        ...
        if sphereIds.isEmpty {
            query = CloudStoreRefManager.instance.vacanciesReferance.whereField("status", isEqualTo: VacancyStatus.Opened.rawValue)
        } else {
            query = CloudStoreRefManager.instance.vacanciesReferance.whereField("status", isEqualTo: VacancyStatus.Opened.rawValue)
                .whereField("sphereId", in: sphereIds)
        }
        
        
        query.whereField("country", isEqualTo: UserDefaults.standard.string(forKey: Constants.USED_COUNTRY) ?? UsedCountry.Kyrgyzstan.rawValue
        )
    ...
    func getNextPageVacancies(){
        ...
        if sphereIds.isEmpty {
            ...
            query = CloudStoreRefManager.instance.vacanciesReferance.whereField("status", isEqualTo: VacancyStatus.Opened.rawValue)
                .whereField("sphereId", in: sphereIds.map { [ $0 ] })
        }
...
extension VacanciesVC: KolodaViewDelegate {
    ...
    func kolodaDidRunOutOfCards(_ koloda: KolodaView) {
        ...
        lastVacancyDocument == nil ? reGetVacancies() : getNextPageVacancies()
    }
    
    func reGetVacancies(){
        let alert = UIAlertController(title: "", message: "noNewVacanciesAlert".localized(), preferredStyle: .alert)
