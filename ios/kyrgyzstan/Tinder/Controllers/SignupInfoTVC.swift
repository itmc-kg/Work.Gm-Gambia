...
class SignupInfoTVC: UITableViewController {
    ...
    private let educationTitle = "education".localized()
    private let experinceTitle = "expiriance".localized()
    ...
    private var employee: Employee?{
        didSet{
            if let employee = employee{
                avatarButton.sd_setImage(with: employee.getImageUrl(),for: .normal, placeholderImage: Constants.placeholder, options: .continueInBackground, completed: nil)
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        switch section {
        case 0:
            label.text = "education".localized()
        case 1:
            label.text = "expiriance".localized()
        default:
            label.text = ""
        }
...
extension SignupInfoTVC: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func addAvatar(_ sender: UIButton) {
        ...
        alert.addAction(UIAlertAction(title: "upload_photo".localized(), style: .default, handler: { (action) in
