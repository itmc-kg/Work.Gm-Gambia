...
class EducationTVCell: UITableViewCell {
        ...
        didSet{
            if let education = education{
                ...
                if let index = Constants.EDUCATION_TYPE_IDS.firstIndex(of: education.type){
                    ...
                    if language == "en" {
                        educationTypeTextField.text = Constants.EDUCATION_TYPES_EN[index]
                    } else if language == "ky" {
                        educationTypeTextField.text = Constants.EDUCATION_TYPES_KG[index]
                    } else {
    ...
    override func awakeFromNib() {
        ...
        let language = UserDefaults.standard.string(forKey: Constants.applicationCurrentLanguage) ?? Constants.RU_lang
        if language == "en" {
            educationTypeTextField.text = Constants.EDUCATION_TYPES_EN[0]
        } else if language == "ky" {
            educationTypeTextField.text = Constants.EDUCATION_TYPES_KG[0]
        } else {
...
extension EducationTVCell : UIPickerViewDataSource{
    ...
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        let language = UserDefaults.standard.string(forKey: Constants.applicationCurrentLanguage) ?? Constants.RU_lang
        if language == "en" {
            return Constants.EDUCATION_TYPES_EN[row]
        } else if language == "ky" {
            return Constants.EDUCATION_TYPES_KG[row]
        } else {
...
extension EducationTVCell : UIPickerViewDelegate{
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        let language = UserDefaults.standard.string(forKey: Constants.applicationCurrentLanguage) ?? Constants.RU_lang
        if language == "en" {
            educationTypeTextField.text = Constants.EDUCATION_TYPES_EN[row]
        } else if language == "ky" {
            educationTypeTextField.text = Constants.EDUCATION_TYPES_KG[row]
        } else {
