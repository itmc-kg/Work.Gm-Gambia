...
//  Tinder
//
//  Created by Azamat Kushmanov on 10/10/18.
...
class ChatVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        textView.text = "sendMessage".localized()
    ...
    @IBAction func sendMessage(_ sender: UIButton) {
        guard let user = Auth.auth().currentUser, let chat = self.chat, textView.text != "sendMessage".localized() , !textView.text.isEmpty else{
            return
        }
    ...
    func setupNavBar(){
        ...
        if let chatImageView = chatNavView.viewWithTag(1) as? UIImageView{
            ...
            chatImageView.sd_setImage(with: chat.getImageUrl(), placeholderImage: Constants.placeholder, options: .continueInBackground, completed: nil)
        }
...
extension ChatVC : UITextViewDelegate{
    func textViewShouldBeginEditing(_ textView: UITextView) -> Bool {
        if textView.text == "sendMessage".localized() {
            textView.text = ""
