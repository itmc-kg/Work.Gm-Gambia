...
//        let country = UserDefaults.standard.string(forKey: Constants.USED_COUNTRY) ?? UsedCountry.Kyrgyzstan.rawValue
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            if let filterLabel = cell.viewWithTag(1) as? UILabel {
                filterLabel.text = "vacancyFilterLabel".localized()
            }
            if let salaryLabel = cell.viewWithTag(2) as? UILabel {
                salaryLabel.text = "salaryLabel".localized()
            }
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if let selectedCountLabel = header.viewWithTag(1) as? UILabel{
            ...
            self.selectedCountLabel.text = "\("selected".localized()) \(selectedSpheres.count)"
        }
        
        if let sphereLabel = header.viewWithTag(2) as? UILabel {
            sphereLabel.text = "sphere".localized()
        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.selectedCountLabel.text = "\(String(describing: "selected".localized)) \(selectedSpheres.count)"
